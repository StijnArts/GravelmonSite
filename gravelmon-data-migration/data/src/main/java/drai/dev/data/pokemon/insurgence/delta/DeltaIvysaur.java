...
public class DeltaIvysaur extends Pokemon {
    ...
    private void setPosingData(boolean isFemale) {
        getPosingFileData(isFemale).setPortraitData( 1.45f, new Vector3(-0.23, -0.47, 0));
        getPosingFileData(isFemale).setProfileData( .9f, new Vector3(0.0, 0.37, 0.0));
        getPosingFileData(isFemale).setBasicHead();
        getPosingFileData(isFemale).setRootBone("ivysaur");
        getPosingFileData(isFemale).setAnimationFileName("deltaivysaur"+(isFemale ? "_female" :""));
        String[] lookAnimations = new ArrayList<>(List.of(
                "q.look('head', 1, 1, 10, -20)",
                "q.look('leaf_front', 1, 0, 3, -13)"
        )).toArray(new String[2]);
        getPosingFileData(isFemale).addMiscAnimations(
                "cry",
                "recoil"
        );
        getPosingFileData(isFemale).addMiscAnimation("faint", "q.bedrock_primary('deltaivysaur"+(isFemale ? "_female" :"")+"', 'faint', q.curve('one'))");


        getPosingFileData(isFemale).addAnimations(List.of(
                AnimationData.battleIdleAnimation().addAnimations(lookAnimations).withBlink(),
                AnimationData.standingAnimation().addAnimations(lookAnimations).withBlink().markAsBackupPose(),
                AnimationData.walkingAnimation().addAnimations(lookAnimations).withBlink(),
                AnimationData.sleepingAnimation()
        ));
        getPosingFileData(isFemale).setFaint("bedrock("+this.getCleanName()+", faint)");
    }
}
